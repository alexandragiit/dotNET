    ...
    public class resAPI
    {
        ...
        public void updateResource(string path, List<string> atr)
        {
            ...
            using (Model1Container1 context = new Model1Container1())
            {
                ...
                var b = context.atributes.Include("resources").Where(x => x.resource.path == path).Select(x => x);
                foreach (var i in b)
                {
                    context.atributes.Remove(i);
                }
                var id = context.resources.Include("atributes").Where(x => x.path == path).Select(x => x.resID);
        ...
        public void deleteResources(List<string> paths)
        {
            using (Model1Container1 context = new Model1Container1())
            {
                ...
                {
                    var b = context.atributes.Include("resources").Where(x => x.resource.path == n).Select(x => x);
                    foreach (var i in b)
                    {
                        context.atributes.Remove(i);
                    }
                ...
                foreach (var n in paths)
                {
                    var c = context.resources.Include("atributes").Where(x => x.path == n).Select(x => x);
                    foreach (var i in c)
                    {
        ...
        public void deleteAtribute(List<string> names)
        {
            using (Model1Container1 context = new Model1Container1())
            {
                ...
                {
                    var b = context.atributes.Include("resources").Where(x => x.name == n).Select(x => x);
                    foreach (var i in b)
                    {
                        context.atributes.Remove(i);
                    }
