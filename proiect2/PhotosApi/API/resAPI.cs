...
namespace PhotosApi.API
{
    public class resAPI
    {
        ...
        public List<resources> getAllResources()
        {
            ...
            {
                var r = context.resources.OrderBy(m => m.added_at).ToList();
        ...
        public List<resources> getAllPhotos()
        {
            using (Model1Container1 context = new Model1Container1())
            {
                var r = context.resources.Where(m => m.type == "image").OrderBy(m => m.added_at).ToList();
        ...
        public List<resources> getAllVideos()
        {
            using (Model1Container1 context = new Model1Container1())
            {
                var r = context.resources.Where(m => m.type == "video").OrderBy(m => m.added_at).ToList();
        ...
        public List<string> getAllAtributes()
        {
            using (Model1Container1 context = new Model1Container1())
            {
                var atr = context.atributes.Select(m => m.name).Distinct().ToList();
                return atr;
            }
        }
        ...
        public List<resources> getResourcesByQueryAtribute(string atr, string query)
        {
            using (Model1Container1 context = new Model1Container1())
            {
                var r = context.atributes.Where(x => x.name == atr).Where(x => x.description == query).Select(x => x.resource).ToList();
